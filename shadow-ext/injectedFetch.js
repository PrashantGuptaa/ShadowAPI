(function () {
  ...
  window.addEventListener("message", (event) => {
    if (event.source === window && event.data?.type === "GET_RULES_RESPONSE") {
  ...
  function matchRule(url = "", method = "", body = "", rule = {}) {
    const { match = "", payload: rulePayloadRaw = [], url: ruleUrl = "", method: ruleMethod = "" } = rule;
    
    // Check URL match
    const urlMatch = match === "PARTIAL_MATCH" 
    ...
    if (rulePayload.length > 0 && body && !["GET", "DELETE"].includes(method)) {
      try {
        ...
        bodyMatch = rulePayload.every(rule => {
          switch(rule.matcher) {
            case "Equals": return parsedBody[rule.key] === rule.value;
            case "Not Equals": return parsedBody[rule.key] !== rule.value;
            case "Contains": return parsedBody[rule.key] && 
  ...
  function overrideFetchAndXHR(rules = []) {
    console.log("[ShadowAPI] Overriding network functions with", rules.length, "rules");
    ...
    window.fetch = async function (input, init = {}) {
      ...
      const matchedRule = rules.find(rule => matchRule(url, method, body, rule));
      
      if (matchedRule) {
        console.log("[ShadowAPI] Mocking fetch for", url);
        const responseData = matchedRule.response || JSON.stringify(matchedRule.mockResponse);
